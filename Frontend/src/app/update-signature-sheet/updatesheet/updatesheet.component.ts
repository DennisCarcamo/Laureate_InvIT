...
export class UpdatesheetComponent implements OnInit {
  ...
  constructor(
    private _searchEmployees: SearchEmployeeService,
    private modalService: BsModalService,
    private cookieService: CookieService,
    private router: Router
  ) {
    this.fullImagePath =
      "/assets/images/blurred-background-close-up-coffee-cup-908284.jpg";
  }
  ...
  updateSheet() {
    ...
    let x = this.selectemploye[0];
    if (
      (this.productremovesevent.length != 0 ||
        this.productsevent.length != 0) &&
      x
    ) {
      this._searchEmployees
        .updateSheet(
        ...
        .subscribe((result) => {
          ...
          if (result["message"] == "Signature Sheet correctly inserted") {
            this._searchEmployees
              .updateSignatureSheet(
                this.productremovesevent,
                this.productsevent,
                this.selectproducts,
                this.selectemploye,
                this.option,
                this.selectemploye[0]["EMPLOYEE_ID"]
              )
              .subscribe((result) => {
                console.log(result);
                alert("updated");
