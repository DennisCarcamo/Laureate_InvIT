...
export class LoanAComponent implements OnInit {
  ...
  continueemployee() {
    ...
    if (x) {
      ...
      if (this.option == 2) {
        this._Service.processValidation(x.EMPLOYEE_ID).subscribe((res) => {
          let m;
          m = res["message"];
          if (m == "null") {
          ...
          if (m["id_type"] != 2) {
            this.validation = 1;
            this._Service.getUserProducts(x.EMPLOYEE_ID).subscribe((res) => {
              console.log(res);
  ...
  openLoanModal(template: TemplateRef<any>) {
    ...
    if (x) {
      this._Service
        .searchLoanSheetsProducts(x.id_signature)
        .subscribe((res) => {
          let x = res["Products"];
          if (x) {
  ...
  openOffboardingModal(template: TemplateRef<any>) {
    ...
    //this.searchText = x.EMAIL
    this._Service
      .searchLoanSignatureSheets(1, x.EMAIL, 10, 3)
      .subscribe((res) => {
        //console.log("############");
        //console.log(res);
        let y = res["meta"];
        ...
        if (this.boolLoanPending) {
          this.messegeError =
            "This user have Loan sheets pending, make sure to release then first";
          document.getElementById("errorModalbtn").click();
        } else {
          this.modalRef = this.modalService.show(template);
        }
      ...
      this._Service
        ...
        .subscribe((res) => {
          ...
          if (res["message"] == "Signature Sheet correctly inserted") {
            this._Service
              .createOffboarding(this.offBoardingProducts, x.EMPLOYEE_ID)
              .subscribe((res) => {
                if (res["message"] == "done") {
