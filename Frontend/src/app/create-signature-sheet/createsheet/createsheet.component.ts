...
export class CreatesheetComponent implements OnInit {
  ...
  constructor(
    ...
    this.fullImagePath =
      "/assets/images/black-coffee-cactus-cellphone-875514.jpg";
  }
  ...
  continueemployee(template: TemplateRef<any>) {
    ...
    if (x.EMPLOYEE_ID) {
      ...
      this.searchEmployeeService
        .processValidation(x.EMPLOYEE_ID)
        .subscribe((res) => {
          m = res["message"];
          console.log(res);
  ...
  insertSheet() {
    ...
    if (x && l != 0 && this.validation == 1) {
      ...
      if (this.validation == 0 && this.option == 1) {
        alert(
          "You cannot create this signature sheet, offboarding sheet is needed"
        );
      }
      if (this.validation == 0 && this.option == 3) {
      ...
      this.searchEmployeeService
        ...
        .subscribe((result) => {
          ...
          if (result["message"] == "Signature Sheet correctly inserted") {
            ...
            //this.createSignatureSheet(x, this.productsevent);
            this.searchEmployeeService
              .saveRelationships(x, this.productsevent)
              .subscribe((result) => {
                console.log(result);
                alert("Done");
                //alert('Signature Sheet successfully created.!');
  ...
  confirm() {
    ...
    if (x.EMPLOYEE_ID) {
      ...
      alert(
        "The employee does not have Employee Id, please make sure to get all the info ready before proceding"
      );
    }
  }
