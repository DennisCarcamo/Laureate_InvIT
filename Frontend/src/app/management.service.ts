...
export class ManagementService {
  ...
  getRoles(){
    return this.httpClient.get(`http://hnlnoitinvprod01.hnsc.net/api/v1/roles`).map(result => result);
  }

  getPrivilegesbyRoles(role){
    return this.httpClient.get(`http://hnlnoitinvprod01.hnsc.net/api/v1/roles_x_privileges/${role}`).map(result => result);
  }
  ...
  getSystemUsers(){
    return this.httpClient.get(`http://hnlnoitinvprod01.hnsc.net/api/v1/usersadmins`).map(result => result);
  }
